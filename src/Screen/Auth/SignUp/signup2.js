...
const SignUpScreen2 = ({ navigation }) => {
    ...
    const handleLinkedInSignUp = async () => {
        try {
            ...
            const redirectUri = 'edupro://callback'; // We'll use this scheme
            const scope = 'r_liteprofile r_emailaddress';
    
            const authUrl = `${linkedInAuthUrl}?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${scope}`;
    ...
    const handleLinkedInCode = async (code) => {
        try {
            ...
            Alert.alert('Error', 'Failed to complete LinkedIn sign-up: ' + error.message);
        }
    };
